...
parser = argparse.ArgumentParser(description='Utility to build and/or tear L2 pseudowires across K-20 on demand.')
# Required arguments
group = parser.add_mutually_exclusive_group(required=True)
parser.add_argument('routerA', help='IP address (loopback) PE headend "A" of the circuit to be built')
parser.add_argument('routerAifc', help='interface of routerA, including vlan, if required (xe-X/Y/Z.###)')
parser.add_argument('routerZ', help='IP address (loopback) PE headend "Z" of the circuit to be built')
parser.add_argument('routerZifc', help='interface of routerZ, including vlan, if required (xe-X/Y/Z.###)')
...
# Optional arguments
parser.add_argument('--username', help='Username to connect to system with if different from  $USER env')
parser.add_argument('--port', type=int, help='Specify if NETCONF is running on a non-standard port (default is 22)')
group.add_argument('--build', dest='build', action='store_true', help='Builds the specified circuit (default)')
group.add_argument('--tear', dest='tear', action='store_true', help='Tears down the specified circuit')
...
localvcid = args.vcid
#build a description for the config
desc = ''.join((bandwidth + "bps" + " L2 circuit: "+routerA, " (ifc: ", routerAifc+ ") to " + routerZ + " (ifc: " + routerZifc + ") using vcid " + localvcid))
...
# Check end point will make sure that the devices at each end of the circuit are reachable
...
def set_end_point(router_a, a_ifc, router_z, z_ifc, bw, vcid, username, portnum):
    ...
    try:
        dev.cu.load(template_path='layer2_circuit_config_template.conf', merge=True, template_vars=tvars, format="text")
...
def set_vlan_id(router_a, a_ifc, vlanid):
    print "\nConfiguring headend router {} ifc {} to match tail vlan-id (vlan-id: {})".format(router_a, a_ifc, vlanid) 
    ...
    print "{}:\tLoading configuration changes as candidate configs".format(router_a)
    try:
        cfg.load("set interfaces {} vlan-id {}".format(a_ifc, vlanid), format="set", merge=True)
    except ValueError as err:
        print err.message
...
def tear_l2_circuit(router_a, a_ifc, router_z, z_ifc):
    ...
    try:
        print "{}:\tRunning \'delete protocols mpls label-switched-path {}\'".format(router_a, lsp_shortname)
        cfg.load("delete protocols mpls label-switched-path {}".format(lsp_shortname), format="set")
        print "{}:\tRunning \'delete protocols neighbor {}\'".format(router_a, router_z)
        cfg.load("delete protocols l2circuit neighbor {}".format(router_z), format="set")
    except Exception as err:
        etree.dump(err.cmd)
        etree.dump(err.rsp)
        sys.exit("Could not load the configuration on {}: {}".format(router_a, err.rsp.findtext('.//error-message')))
    try:
        print "{}:\tCommitting the configuration".format(router_a),
        cfg.commit()
        print " Success!"
...
# Check to make sure that the user isn't trying to set up a circuit between two 
...
if build:
    ...
    # Convert the bandwidth to bits per second as required by the junos and validate input a bit
    ...
    if str(bandwidth[-1]).lower() == 'g':
        ...
        sys.exit("Bandwidth must be specified in Mb/s (M) or Gb/s (G). You entered %s" %bandwidth)
    if bps > 10000000000:
        sys.exit("Layer 2 circuits in excess of 10Gb/s should not be provisioned using this utility.")

    # Do stuff
    print "This script will attempt to build a layer 2 circuit with the following criteria:"
            ...
            for router in routerA, routerZ:
                try:
                    if check_end_point(router, username, portnum):
                        ...
                        print "One or more endpoints could not be contacted. Aborting"
                        sys.exit()
                except Exception as err:
                    print err.message
        else:
    ...
    try:
        vlanID = get_vlan_tag (resolvehostname(routerZ), username, portnum, routerZifc)
        print "Success. Vlan ID is {}".format(vlanID)
    except Exception as err:
    #     print "Could not retreive vlan ID from {}.".format(routerA)
        sys.exit("Could not retreive vlan ID from {}.".format(routerA))
    
    print "\nConfiguring l2circuit and RSVP reservation on headend router: {}".format(routerA)
    try:
        set_end_point (resolvehostname(routerA), routerAifc, resolvehostname(routerZ), routerZifc, str(bps), localvcid, username, portnum)
    except Exception as err:
        print err.message
    print "\nConfiguring l2circuit and RSVP reservation on tailend router {}".format(routerZ)
    try:
        set_end_point(resolvehostname(routerZ), routerZifc, resolvehostname(routerA), routerAifc, str(bps), localvcid, username, portnum)
    except Exception as err:
        print err.message
    try:
